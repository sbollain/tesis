...
```{r}
#Creamos un dataframe para guardar si hay valores de cada parámetro y cuantos hay
valores <- data.frame(etiquetas,0,FALSE)
colnames(valores) <- c("Parámetros","Cantidad","Existen") 

for (i in 1:nrow (valores)){
        valores$Cantidad[i] <- nrow (mydata[mydata$parametro_id==etiquetas[i],])
        if ( valores$Cantidad[i] > 0) {
                valores$Existen[i] <- TRUE
                ...
                print(paste("Hay", valores$Cantidad[i] ,"valores de", (etiquetas[i]),"entre", serie_d$fecha[1], "y", serie_d$fecha[valores$Cantidad[i]]))
                
                assign (etiquetas[i], xts(serie_d$valor,round(serie_d$fecha,"secs")))
...
```{r}
Datos_ts <- merge.xts (FR_PULSO,FCECG,SAT_O2,TAS,TAM,TAD,FR_RESPIRAT,FR_P,FR_E,ET_CO2,PTC_CO2,INS_CO2,PIP,PMVR,VTE,PS_VOL,COMPLIANZA,RESISTENCIA) 
```
